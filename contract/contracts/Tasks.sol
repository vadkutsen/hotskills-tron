...
contract Tasks is Ownable, ReentrancyGuard {
 ...
 enum TaskTypes {
    ...
    enum Statuses {
        Active,
        Assigned,// SPDX-License-Identifier: MIT
pragma solidity 0.8.11;
...
contract Tasks is Ownable, ReentrancyGuard {
 ...
 function assignTask(uint256 _id, address payable _candidateAddress)
        ...
        returns (bool)
    {
        ...
        require(success, "Tranfer failed.");
        delete tasks[_id];
        delete allTasks[tasks[_id].allTasksIndex];
        allTasks[tasks[_id].allTasksIndex] = allTasks[
            allTasks.length - 1
        ];
        allTasks.pop();
    ...
    modifier inStatus(Status _expected_status){
        require(status == _expected_status);
        _;
    }
